...
1. Does the artifact's content match its declared `type` in frontmatter?
   ...
   - `design-proposal`: specifies a Ports 2.0 upgrade; does not re-score the corpus.
...
Identify out-of-scope sections by heading, and any undeclared or mixed scale. Recommend: move
to a separate artifact, remove, declare the scale, or amend the spec to accommodate.

## What NOT to do

Do not evaluate the substance of claims. Do not flag content as out-of-scope just because it is
inconvenient.
