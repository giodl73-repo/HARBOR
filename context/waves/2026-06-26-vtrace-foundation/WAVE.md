...
Establish HARBOR's VTRACE planning baseline before any implementation. Author the VTRACE
...
HARBOR is the Ports 2.0 build, seventh in the infrastructure-network portfolio after ROUTE
(highways), PYLON (grid), GAUGE (rail), BASIN (water), PACKET (internet), and TARMAC (air) —
completing the freight/transport modes (road/rail/air/sea) and, like BASIN/PACKET/TARMAC,
explicitly multi-scale. Doing the V-model left side first means the implementation is governed
by accepted work packages. The invariant this wave establishes: every downstream HARBOR claim
...
|---|---|---|---|---|---|---|
| MISSION | `docs/vtrace/MISSION.md` | settled | full panel (real `.roles`) | 1 minor (terminal/alliance control) — applied | fixed point reached | CONOPS |
...
| REQUIREMENTS | `docs/vtrace/REQUIREMENTS.md` | settled | full panel (real `.roles`) | 1 minor (demand/access basis) — applied as REQ-007 | fixed point reached | SPECIFICATION_BASELINE |
| SPECIFICATION_BASELINE | `docs/vtrace/SPECIFICATION_BASELINE.md` | settled | full panel (real `.roles`) | 1 minor (throughput read as control-free) — applied | pass_with_risk; fixed point | TRACE |
