# Pulse 05: WP-005 `harbor-gap` gap analysis (scale-filtered)
...
require an explicit marker (REQ-016). Integrates tier-SLA gaps from `harbor-tier`. Creates
`crates/harbor-gap/`.

Parent: REQ-008/016 · SPEC-006/013 · PKG-005 · CR-004.

Exit: `cargo test -p harbor-gap` green (one found gap at a scale, one null-result case, and
