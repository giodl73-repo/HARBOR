...
VTRACE adoption scope: define verification methods and command levels for HARBOR's requirements.
HARBOR is greenfield, so implementation-dependent checks are `pending` (no code/corpus to run
...
| REQ-004 | schema check / inspection | validate corpus frontmatter keys against schema | stable port/terminal/lane/network id present; labels are not keys | pending | future EVID-004 |
...
| REQ-007 | analysis / inspection | inspect throughput/dwell claims for named demand + access basis | surge-vs-average and tide-restricted-vs-all-tide basis named on each claim | pending | future EVID-007 |
...
| REQ-010 | role review | confirm throughput/dwell/resilience/channel/hinterland/competition/environment/cost/control lenses represented | stakeholder lenses present in `.roles/` and applied | pass_with_risk | EVID-010 (`.roles/` panel built) |
...
| REQ-012 | git inspection | `git status --short`; confirm no TRACKER pointer touched | HARBOR changes stay in the child repo | passed | EVID-012 |
...
# Implementation levels (planned — once harbor-network/CLI exist)
# cargo fmt --check
# cargo test
# harbor corpus regenerate ; harbor tier-sla --gate ; harbor gap --scale national
```
...
(process/doc) from pending (implementation). No unresolved critical/major finding.
