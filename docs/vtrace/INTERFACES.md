...
Interface type: target (greenfield). Controls HARBOR's external and cross-layer boundaries so
future work packages cannot change them silently. IF-001..004 restate the
`SPECIFICATION_BASELINE.md` public contracts; IF-005..006 add the crate API and CLI introduced in
`ARCHITECTURE.md`. None are implemented yet.
...
| IF-005 | `harbor-network` library API | API (Rust crate) | PKG-001 | PKG-002..006 | Public types/functions semver; breaking change is change-control | future VER-007 / cargo test |
| IF-006 | `harbor` CLI | CLI | PKG-006 | maintainers, agents, analysts | Subcommands/flags additive (incl. `--scale`); output schemas versioned | future VER-001 / command review |

## Interface Details

### IF-001: Corpus entry file

Purpose: one port/freight-network element as a reviewable, scored, labelled, **scale-tagged**
record.

Inputs: frontmatter (`id`, `type`, `scale`, `market`, `termini`, `tier`, `sla`, source rows) + a
dimension-score block (DIM-01..13).

Outputs: a `validated`/`held`/`draft` corpus artifact joinable by `id`, filterable by `scale`.

Errors: missing `id`, uncited quantity, or missing `scale` → held (REQ-005); type/scope/scale
drift → Scope Keeper finding.
...
Errors: cited quantity with no registry entry → Citation Auditor finding; AIS/nominal used as
observed → must be labelled proxy.
...
Errors: retiring/adding a `DIM-*` or changing weights without a version bump → change-control
violation. Versioning: explicit version + rationale (REQ-006). Evidence: future VER-006.

### IF-004: Tier/SLA record

Purpose: control the T1–T4 tier definitions and per-tier SLA terms.
Inputs: tier definitions, SLA terms (throughput, dwell, connectivity, access), element tier
assignments.
...
violation. Versioning: tier set + SLA terms versioned. Evidence: future VER-014/015.

### IF-005: `harbor-network` library API

Purpose: the graph primitive every layer builds on.
Inputs: ports, lanes with attributes and identity.
Outputs: graph queries — connectivity, centrality (DIM-04), diverse-path existence,
throughput/dwell helpers (with typed demand basis).
...
| Citation Auditor | IF-002 hardens the citation boundary; AIS-as-proxy rule explicit. | pass |
| Maritime Engineer | IF-005 exposes connectivity/throughput as typed queries with error handling, not panics. | pass |
