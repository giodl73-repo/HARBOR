...
VTRACE adoption scope: describe the operating scenarios that HARBOR's requirements and
specification baseline must preserve. CONOPS is the bridge from mission needs (`MISSION.md`) to
observable repo workflows. It asserts no scores, gaps, or projects. HARBOR is greenfield, so
these scenarios describe intended operation, and each names the mission need it serves. Every
scenario is **scale-aware**.

## Actors

| Actor | Responsibility | Needs |
|---|---|---|
| HARBOR maintainer | Own repo truth, active goal, generated artifacts, and promotion posture at a stated scale. | Clear commands, evidence labels, review gates, scoped child-repo changes. |
...
| Freight/maritime analyst | Inspect corpus scores, gap maps, and design options at a chosen scale. | Reproducible artifacts with source, confidence, evidence posture, and scale. |
...
| Stakeholder reviewer | Apply planner, engineer, operations, economist, hinterland, environmental, and port-authority/alliance lenses. | Claims that name throughput, demand basis, resilience, hinterland access, and control posture. |

## Scenarios

### OPS-001: Add And Score An Existing Network (serves NEED-001, NEED-002, NEED-004, NEED-008)

Trigger: a maintainer or agent adds an existing port/freight-network element to the corpus.

Inputs: public source data (BTS / USACE / UNCTAD / AIS), the dimension pool, the corpus schema,
`data/sources.md`, and the element's **scale** and market.

Normal path:

1. Create one corpus file with a stable port/terminal/lane/network identifier and a declared
...
source-needed label and a next evidence step; it is not filled with uncited prose or an assumed
scale.
...
Failure or degraded path: if the corpus is too small or unbalanced, the pass is deferred and the
limitation recorded rather than forcing a change.
...
2. Identify under-served regions (e.g. high dwell + shallow channel + thin hinterland access).
3. Record candidate gaps with the dimensions, corpus comparison, and scale that define them.

Failure or degraded path: if a port system is already fluid, deep, and well-connected, that null
...
Trigger: a conceptual Ports 2.0 project/feature package is proposed for downstream use.

Inputs: the proposal, evidence labels, scale, parliament/editorial lenses, non-goal constraints
from `MISSION.md`.
...
2. Run the 7-voice parliament; require each voice to challenge or accept — including the
   port-authority/alliance realist on any terminal/alliance/labor assumption.
3. Run the 3-role editorial gate (citation, scope incl. scale, numeracy).
4. Promote only the bounded claim; keep construction readiness, concession/permit determination,
   and endorsement out of scope.

Failure or degraded path: if evidence, review, or scope is insufficient, the claim stays held or
downgraded with a next evidence step.
...
Trigger: a maintainer asks to advance HARBOR's VTRACE baseline.
...
Failure or degraded path: if repo state is dirty or out of scope, keep edits scoped to the one
deliverable and report status.
...
Inputs: element attributes (role, throughput, channel, hinterland), the T1–T4 tier model, and
the per-tier SLA contract.

Normal path:

1. Classify the element into T1 (International Gateway Port), T2 (National Port), T3
   (Regional/Inland Port), or T4 (Local Terminal/Berth).
2. Look up the tier's SLA (throughput, dwell, connectivity, access).
3. Assess conformance: does the element meet its tier SLA, with the demand basis named
   (peak-season surge vs average; tide-restricted vs all-tide, REQ-007)?
...
4. If a cross-scale relationship matters (e.g. a gateway-port gap affecting an inland region),
   record it as an explicit, labelled cross-scale note (CON-007).
...
- HARBOR is greenfield: VTRACE is authored ahead of implementation, so scenarios describe
  intended operation, not retrofit.
- The active VTRACE sequence is MISSION → CONOPS → REQUIREMENTS → SPECIFICATION_BASELINE before
  implementation planning.
- Some data sources may be lagged, aggregated, or AIS-noisy; HARBOR records intended acquisition
  and validation even when a full pass is deferred.
- `.roles` review is part of HARBOR operations and must change evidence posture, claim labels, or
  next steps when it finds a gap.
- TRACKER remains the portfolio snapshot repo; HARBOR owns repo-local implementation and VTRACE
  artifacts.
...
| Numeracy Checker | No arithmetic, units, score ranges, dwell, or cost figures. | pass |
| Port-System Planner / Operations Officer | Scoring and review scenarios require demand-basis evidence before promotion. | pass |
| Port-Authority & Alliance Realist | OPS-004 explicitly requires the realist to challenge terminal/alliance/labor assumptions. | pass |
...
Fixed-point note: one actionable finding (source-label/scale custody under-specified) was raised
and applied. No unresolved critical or major finding remains.
...
| OQ-003 | What demand basis (peak-season surge vs average; tide-restricted vs all-tide) anchors throughput/dwell scoring? | Defer to `SPECIFICATION_BASELINE.md`. |
