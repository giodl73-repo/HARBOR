...
Implementation baseline: the accepted left-side VTRACE artifacts. HARBOR is greenfield; this plan
...
Build the pipeline primitive-first. The port/freight graph kernel (`harbor-network`) has no
...
|---:|---|---|---|---|
| 1 | Port/freight graph kernel: identity, connectivity, centrality, throughput/dwell helpers | `Cargo.toml`, `crates/harbor-network/` | WP-001 | Primitive; no deps; carries invariants. |
| 2 | Corpus + scale/market tags + source registry + schema + evidence labels | `crates/harbor-corpus/`, `corpus/SCHEMA.md`, `data/sources.md` | WP-002 | Depends on kernel types; adds scale; feeds all scoring. |
| 3 | Dimension scoring (DIM-01..13) + rubric version record | `crates/harbor-score/` | WP-003 | Depends on corpus; needed before tier/gap. |
| 4 | Tier classification (T1–T4) + SLA conformance (DIM-13) + tier-SLA gap | `crates/harbor-tier/` | WP-004 | Depends on score + kernel. |
| 5 | Gap analysis (under-served regions, scale filter, null result) | `crates/harbor-gap/` | WP-005 | Depends on score + tier. |
| 6 | `harbor` CLI orchestration (incl. `--scale`) + reproducible artifacts | `crates/harbor-cli/` | WP-006 | Orchestrates all layers; last. |
...
|---|---|---|---|
| PKG-001 / IF-005 | WP-001 | `crates/harbor-network/**`, workspace `Cargo.toml` | no |
| PKG-002 / IF-001 / IF-002 | WP-002 | `crates/harbor-corpus/**`, `corpus/`, `data/` | yes (kernel types) |
| PKG-003 / IF-003 | WP-003 | `crates/harbor-score/**` | yes (corpus) |
| PKG-004 / IF-004 | WP-004 | `crates/harbor-tier/**` | yes (score, kernel) |
| PKG-005 | WP-005 | `crates/harbor-gap/**` | yes (score, tier) |
| PKG-006 / IF-006 | WP-006 | `crates/harbor-cli/**` | yes (all) |
...
Active wave: a new `context/waves/<date>-harbor-implementation/` wave (created when WP-001
...
each crate is exercised by its own tests. No external services are required for WP-001; data
acquisition (FLETCH) is introduced only when the corpus is populated, after WP-002 establishes the
schema and scale tags.
...
|---|---|---|---|---|
| WP-001 | Port/freight graph kernel | `crates/harbor-network` | `cargo test -p harbor-network` | evidence/trace/review/status rows |
| WP-002 | Corpus + scale + sources + schema | `crates/harbor-corpus`, `corpus/`, `data/` | `cargo test -p harbor-corpus`, `proof check .` | evidence/trace rows |
| WP-003 | Dimension scoring | `crates/harbor-score` | `cargo test -p harbor-score` | evidence/trace rows |
| WP-004 | Tier + SLA conformance | `crates/harbor-tier` | `cargo test -p harbor-tier` | evidence/trace rows |
| WP-005 | Gap analysis (scale-filtered) | `crates/harbor-gap` | `cargo test -p harbor-gap` | evidence/trace rows |
| WP-006 | CLI orchestration | `crates/harbor-cli` | `cargo run -p harbor-cli -- demo`, `cargo test` | evidence/trace rows |

Boundary rule: VTRACE/work-package/proof/readiness concepts are **not** product features. HARBOR
...
| L2 | Pipeline readiness | `harbor` end-to-end run at a chosen scale on a seed corpus + role review | first public claim |

## Risks

| Risk ID | Risk | Mitigation | Owner |
|---|---|---|---|
| RISK-001 | Scope creep inside a work package. | Each WP has fixed exit criteria + boundary control. | HARBOR maintainer |
| RISK-002 | Premature scoring before calibration. | WP-003 ships provisional scores, labelled; calibration is a later wave. | HARBOR maintainer |
| RISK-003 | Data acquisition / AIS noise bleeds into WP-002. | WP-002 ships schema + fixtures only; real acquisition (FLETCH) deferred; AIS labelled proxy. | data steward |
| RISK-004 | Scale nesting (DEF-005) undecided. | WP-002 ships a flat `scale` enum; nesting is a later change-controlled extension. | HARBOR maintainer |
...
the corpus and gap layers. WP-001 may start. Validation depth (`VALIDATION.md`) is deferred until
