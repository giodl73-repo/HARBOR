...
Repo: HARBOR (all `harbor-*` crates)

Risk level: medium (research tooling with critical graph/identity/label/scale logic; no
safety-of-life, but wrong throughput/dwell/SLA claims are harmful if overstated).
...
|---|---|---|---|---|
| CR-001 | Functions stay small enough for focused review; soft cap 60 logical lines. | All `harbor-*` code | review + `cargo clippy` (`too_many_lines`) | Larger units need a rationale comment. |
...
| CR-006 | No `unwrap`/`expect`/`panic!` in library code paths except tests and documented invariants. | `harbor-network/corpus/score/tier/gap` libs | `cargo clippy`, review | Allowed in `harbor-cli` top-level error reporting and tests. |
...
| Area | Rule | Rationale |
|---|---|---|
| Errors | Each lib crate defines a `thiserror` error enum; `harbor-cli` uses `anyhow`. | Typed library errors (CR-003), ergonomic CLI. |
| Numbers | Throughput = TEU/tons, dwell = days/hours, depth = feet/meters; never conflate; scores are `f64` in `[0,10]`. | Numeracy discipline (editorial gate). |
...
| Demand basis | Throughput/dwell values carry their basis — demand (surge/average) and access (tide-restricted/all-tide) — as typed data, not prose. | Enforce REQ-007/SPEC-DB-01 at the type level. |
...
| Operations & Logistics Officer | The demand-basis tailoring rule pins demand + access into the type system, not prose. | pass |
