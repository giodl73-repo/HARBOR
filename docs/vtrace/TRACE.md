...
VTRACE adoption scope: connect HARBOR's accepted requirements to mission needs, CONOPS scenarios,
controlled specification items, future design surfaces, work packages, verification, validation,
and evidence. HARBOR is greenfield: design elements, work packages, and evidence are
...
| REQ-004 | NEED-004 / CON-002 / OPS-001 | SPEC-001 | port/freight kernel (`harbor-network`) | deferred → IMPL_PLAN | VER-004 / schema check | OPS-001 | future EVID-004 | accepted |
...
| DEF-002 | Observed (BTS/terminal) vs AIS/nominal dwell modelling. | `SPECIFICATION_BASELINE` (SPEC-DB-02) | deferred |
...
| SPEC-UNK-002 | AIS/nominal vs observed terminal data for DIM-02/03. | operations reviewer | accept risk (proxy) |
| SPEC-UNK-003 | Benefit-cost basis for DIM-12. | maritime economist | defer to calibration |
| SPEC-UNK-004 | Per-tier SLA thresholds (DIM-13). | HARBOR maintainer | defer to calibration |
| SPEC-UNK-005 | Scale nesting representation. | HARBOR maintainer | defer (DEF-005) |
...
- No `VER-*` evidence exists yet; verification methods are named but unrun. The next stage
  ...
  packages run.
- No `WP-*`, `ARCHITECTURE`, `INTERFACES`, or `CODE_RIGOR` artifacts exist yet; the trace marks
  these `deferred`, not satisfied.
- Dimension weights, SLA thresholds, and scale nesting are provisional; the trace does not claim
  calibrated values.
...
| Operations & Logistics Officer | REQ-007/REQ-015 trace the demand basis and tier-SLA gating through SPEC-005/SPEC-012. | pass |
| Port-System Planner | NEED-007/008 trace cleanly to REQ-014/015/016 and SPEC-011/012/013. | pass |

Fixed-point note: no actionable finding required a change; the matrix's value is exposing the
greenfield gaps (no `WP-*`/`EVID-*` yet), recorded as `deferred`/`planned` rather than fabricated.
